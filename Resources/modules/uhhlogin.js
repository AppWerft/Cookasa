exports.tryall = function(_user, _callback) {
	...
	//	this.netstorage(_user, _callback);
...
exports.campusnet = function(_user, _callback) {
	var doResponse = function() {
		if (this.getResponseHeader('refresh')) {
			...
			var regex = /<h1>Herzlich Willkommen,   (.*?)!<\/h1>/mi;
			if (regex.test(this.responseText)) {
				_callback({
					success : true,
					type : 'campusnet',
					name : regex.exec(this.responseText)[1]
				});
			} else {
				_callback({
					success : false,
...
exports.netstorage = function(_user, _callback) {
	var xhr = Ti.Network.createHTTPClient({
		onload : function() {
			if (this.status === 200)
				_callback({
	...
	var xhr_post = Ti.Network.createHTTPClient({
		onload : function() {
			...
			var xhr = Ti.Network.createHTTPClient({
				onload : function() {
					var elems = this.responseText.match(/<input (.*?)\/>/gim);
					console.log(elems);// //cgi-bin/kenninf
					/*AIXSYS	AIXNIS
PWCHECK	9141914
PWCHECK	jwlRdFvloEM
PWCHECK	VJYojcWBsJ.
